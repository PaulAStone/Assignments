...
To address this question, we are going to introduce a new data set derived from bacterial 16S rRNA gene sequences, which were collected from multiple plots at the KBS Long-Term Ecological Site(http://lter.kbs.msu.edu/).
...
Your function will work with the site-by-species matrix.
...
In the R code chunk below, please do the following:
1. Write a function to construct an RAC.
2. Be sure to remove species that have zero abundances.
3. Order the vector from greatest (most abundant) to least (least abundant).
...
Do they agree? If so, why? If not, why not?
...
2. compare this estimate with the output of `vegan's` diversity funciton.
...
2. calculate boht the inverse (1/D) and 1 - D,
2. compare this estimate with the output of `vegan's` diversity funciton.
...
Next, Let's visualize our results by plotting the empirical RAC and the predicted RAC for each model:
...
***Question 11***: Below, write a short paragraph summarizing vegan's output/results in your own words.  
...
***Question 12***: Why is it important to account for the number of parameters a model uses when judging how well it explains a given set of data?

> ***Answer 12***:



***Question 13***: a.) What does the preemption model assume about the relationship between total abundance (N) and total resources that can be preempted? b.) Why does the niche preemption model look like a straight line in the RAD plot?

> ***Answer 13a***:  
> ***Answer 13b***:


***Question 14***: What is your favorite diversity or evenness metric covered so far, and why? What was the most confusing?

> ***Answer 14***:


***Question 15***: What is your favorite species abundance model and why?
