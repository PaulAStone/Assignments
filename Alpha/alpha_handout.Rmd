...
In this excercise, we will rely heavily on a contributed R package called `vegan`. This package contains tools commonly used in ecological research, including analyses of $\alpha$-diversity.
...
If we go back to the rank abundance curve, we can retrieve additional information, including the forces, processes, and/or mechanisms influencing the structure of our ecologicl system.
...
From the output, you can see that `vegan` fits five models (*Null*, *Preemption*, *Lognormal*, *Zipf*, and *Mandelbrot*) using the rank-abundance curve.
(`vegan` refers to it as a rank-abundance distribution (RAD)).
...
AIC and BIC can be used for comparing multiple model testing. 
In other words, which model is best supported by my data? 
...
> ***Answer 11***:
### Not sure about the question above; how about incorporating information below into question.
...
**But, what should we conclude from this curve fitting exercise? That is, what do these models represent?** ##----> Turn this into a question or delte##
...
***Question 12***: Answer the following questions about the preemption model: ##--> perhaps move below?##
...
#### Zipf:
The Zipf model is based on Zipfs Law, an well-known observation that many types of ranked data are fit by a simple scaling law (a.k.a., power law).
...
Calculate Simpson's D, 1 - D, and Simpson's inverse (i.e. 1/D) for site 1 of the BCI site-by-species matrix. ###I don't understand the question###

2.  Along with the rank-abundance curve (or rank-abundance distribution), another way to visualize the distribution of abundance among species is with a histogram (a.k.a., frequency distribution) that shows the frequency of different abundance classes.
