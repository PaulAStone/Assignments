...
rm(list=ls()) 
getwd() 
setwd("~/GitHub/QuantitativeBiodiversity/Assignments/Alpha") #Change Assignments
```

### Install Packages

In this excercise, we will rely heavily on a **contributed R package** called `vegan`. `Vegan` contains tools commonly used in ecological research, including analyses of $\alpha$-diversity. 
...
One index that provides an estimate of how well a site is sampled is known as **Good's Coverage (C) where** $C = 1 - \frac{n_{1}}{N}$, where $\bf{n_{1}}$ is the number of **singleton species** (species only detected once), and $\textbf{N}$ is the total number of individuals in the sample.
Examining the equation for Good's C reveals that the fraction is simply the portion of $\textbf{N}$ represented by singleton species.
Subtracting this from 1 give the portion of $\textbf{N}$ belonging to species sampled more than once.
...
d.  What would we conclude from Good's Coverage if $\bf{n_{1}}$ equaled $\textbf{N}$?
...
For most ecological systems, sample size is much smaller than $\textbf{N}$ and many taxa can easily go undetected. 
...
In the following R chunk, we load this microbial dataset as a matrix and identify a sample (i.e., site) for subsequent analysis. 
...
In thie function, the first argument is site as either row number or row title (e.g., 1 or "T1_1") and the second argument is the site x species matrix (e.g., soilbac):
...
A common way to reduce the bias associated with different $\textbf{N}$ and to compare sites with both different $\textbf{N}$ and different $\textbf{S}$, is to **rarify** samples down to a "lowest common denominator".
For example, if there are 2 sites, one with $\textbf{N = 100}$ and one with $\textbf{N = 50}$, we could randomly sample (without replacement) 50 individuals from the site with greater $\textbf{N}$.
Generating many random samples (each of 50 individuals) from the larger site will allow us to calculate the mean (i.e. expected $\textbf{S}$) and standard error. Likewise, we'll be able to tell whether the $\textbf{S}$ of the smaller sample falls within the confidence-intervals for expected $\textbf{S}$ of the larger sample and hence, whether the difference in $\textbf{S}$ between the two sites is simply due to difference in $\textbf{N}$.
...
S.plot <- barplot(S.rarefy[1, ], xlab = "Treatment", ylab = NULL, 
                  ...
                  pch = 15, las = 1, cex = 1.25, cex.lab = 1.4, cex.axis = 1.25)
