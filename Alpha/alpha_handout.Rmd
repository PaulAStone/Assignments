...
To address this question, we are going to introduce a new data set derived from bacterial 16S rRNA gene sequences for bacteria, which were collected from multiple plots at the KBS Long-Term Ecological Site (http://lter.kbs.msu.edu/).
...
Chao2 is commonly used when estimating richness with clonal organisms and/or percent cover data (e.g., corals).
...
Becasue sites with greater abundance generally have greater species richness, differences in richness may largely be due to differences in sample size.

A common way to reduce the bias associated with different *N* is to **rarify** samples down to a "lowest common denominator".
...
***Question 8***: What effect does log-transforming species abundance data have on how we interpret evenness in a RAC?
...
Let's examine the value of evenness for `site1` using a different evenness metric that is supposedly less biased in this way.
...
Dividing 2 by $\pi$ times the arctangent forces the result to take values between 0 and 1.
...
In the end, the $E_{var}$ function can be written as follows:
...
This model adds one parameter ($\beta$) to the Zipf model. The expected abundance of a species (*a*) at rank *r* is $a_r = N \cdot c \cdot (r + \beta)^{\gamma}$. 
