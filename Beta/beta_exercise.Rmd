...
In the R code chunk below, do the following:  
1. write a function for calculating Whittaker's $\beta$, $\beta_{w}$, and  
...
2. define the order of sites in the River, and  
...
In the R code chunk below, do the following:  
1. identify influential species based on correlations along each PCoA axis using a correlation-coefficient cutoff of 0.70, and   
...
In the R code chunk below, do the following:  
1. Create a vector the identifies fish habitat quality along the Doubts River.  
...
In the R code chunk below, do the following:   
1. Create distance matrices for both fish communities and environmental factors.  
2. Use a Mantel test to determine if these matrices are correlated.  
...
2. Conduct a Canonical Correspondance Analysis using the environmental matix as the explainatory varaibles.  
...
6. Plot the constrained ordination results including labeled points for each site.  
