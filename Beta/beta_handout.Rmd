...
**Seems to me that when we're referring to the dataset, "d" should be lowercase: `doubs`; this is how it's loaded. I stoped correcting here**
...
However, there are numerous methods for clustering (e.g., Single Linkage, UPGMA, UPGMC), and the choice of clustering method can influence the conclusions that you draw from your analysis.  
...
The primry aim of ordination is to represent multiple variables (e.g., sites) in a reduced number of orthogonal (independent) axes. 
The first axis of an ordination explains the most variation in the data set, followed by the second axis, then the third, and so on (there are typically n-1 axes, where n is the number of sites).
Ordination plots can be particularly useful for visualizing the similarity among objects. 
...
When choosing an ordination technique, carefull consideration should be given to the data type (continuous vs. categorical), model assumptions, and the underlying mathematical procedures that are invovled.

In this exercise, we focus on **Principal Coordinates Analysis (PCoA)**, which is sometimes referred to as metric multidimensional scalling. 
...
The dimensionality of $\pmb{A}$ is then reduced by determining each eigenvector, $\pmb{u}_i$, and eigenvalue, $\pmb{\lambda}_i$, which solves the following equation: $\pmb{A u}_i = \pmb{\lambda}_i \pmb{u}_i$. 
...
To conduct a PCoA, we will use the `cmdscale` function from the `stats` package in R, which performs PCoA (a.k.a., "classical metric multidimensional scaling").
...
The eigenvalues can also be used to inform us about how much variation a PCoA was able to capture in a data set. 

In PCoA, we can quantify the amount of variation that is explained by each orthogonal axis, which provides an overall sense of how much "signal" is our data set.
We can estimate the variance explained by dividing the eigenvalue of each axis by the sum of all eigenvalues. 
...
```{r}
explainvar1 <- fish.pcoa$eig[1]/sum(fish.pcoa$eig)
explainvar2 <- fish.pcoa$eig[2]/sum(fish.pcoa$eig)
explainvar3 <- fish.pcoa$eig[3]/sum(fish.pcoa$eig)
sum.eig <- sum(explainvar1, explainvar2, explainvar3)
```

Another way to evaluate our analysis is to make sure that the first few PCoA axes capture a greater than expected amount of the total possible variation.
There are two ways we can make this assessment. 
...
**I'm not sure whether or not student are going to intuit broken stick model without an explanation**
...
**Can/should this be turned into a question? -- perhaps make them use percent variaiton, avg eig, and broken stick model to evaluate this PCoA**Qualitatively, we can see that the first three axes of our PCoA ordination have higher eigenvalues than both the mean eigenvalue and the expectation of the Broken-Stick model. 
Based on this, we can be comfortable using these as a representation of our data. By qualitative assessment, we can see that the first three axes of our PCoA ordination have higher eigenvalues than both the mean eigenvalue and the expectation of the Broken-Stick model. 
Based on this, we can be comfortable using these as a representation of our data.

Finally, we will use some of the plotting functions in `vegan` to visualize the ordination. 

```{r}
ordiplot(scores(fish.pcoa)[,c(1,2)], type='t', main="PCoA with taxa")
abline(h=0, lty=3)
abline(v=0, lty=3)
```
...
One useful feature of ordinations is the ability to plot expainatory vectors in your reduced ordination space. For example, we might be interested in how individual fish species contribute to the patterns observedin our PCoA. 
