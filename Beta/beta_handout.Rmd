...
To date, we have analyzed biodiversity data sets for freshwater zooplankton, tropical forest trees, and soil bacteria.
...
Because we have a wealth of information about this dataset, we can use that information for various purposes.
We might use the environmental or spatial data to test a hypothesis.
However, we can also use the spatial data for graphical representations. 
For example, below we have provided a two plots of the doubs fish data. 
The first plot shows richness at each site in the stream.
...
In the last exercise, we used species abundance data and the *site-by-species* matrix to measure the diversity within a particular site (e.g., richness). 
However, these measurs are not always adequate for compariing communities across site.
For example, in the figure above you can see that there is a stretch of the Doubs River where observed richness is constant at 6 species.
However, if you look at just Brown Trout abundance you can see large changes in the abundance of this particular species in the same stretch of the river. 
Therefore, to properly make comparisions between sites, we will need to make sure that we are comparing communities based on compositional similarities and differences. 
