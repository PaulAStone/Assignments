...
b.  What are the units of nitrate ("nit") in the stream water?
c.  How many fish species are there in the `doubs` list?
...
These indices allow us to estimate $\beta$-diversity for two samples. 
...
To move beyond indices that calculate $\beta$-diversity for two samples, we are now going to introduce another primary ecological data structure: the the **Resemblance Matrix**. 
