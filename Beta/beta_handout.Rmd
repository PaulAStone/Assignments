...
d.  How many fish species are there `doubs`?
...
Throughout this handout, we adopt the notations of Legendre & Legendre (2012), which can be electronically accessed via the IU library (see the course syllabus [http://goo.gl/y4oK7c]). 
...
###Creating a PCoA ordination plot 
...
***Question 8***: As you will recall, the fish assemblages fromteh Doubs River came from sites that were ordered in space with *Site 1* being the most upstream and *Site 30* being the most downstream (see plot in section 2). Given this, what hypotheses could you make about these fish assemblages? (Reminder: the original goals of this study included using fish communities as indicators of stream health). 

**I'm not sure I really understand what we want students to say about question 8. I assume it is intended to relate to the ordination, but the question seems a little divorced from this. How about incoporporting into question 9?

> ***Answer 8***:


###Identifying influential species in PCoA 
One useful feature of ordinations is the ability to plot explanatory vectors in ordination space. 
For example, we might be interested in how differences in species abundances contribute to the separation of samples along each PCoA axis. 
...
We will obtain this information using the `add.spec.scores()` function in the `BiodiversityR` package.
First, we will use this function to calculate species coordinates in our PCoA.
These coordinates reflect how (strength and direction) the species contribute to ordination patterns.
Then we will use the same function to determine the correlation of each species along each axis.
We can use some cut-off criteria to pull out the important species.
...
# Calculate Species Correlations and Identify Important Species
...
***Question 9***: Using the correlations above, can you generate any hypotheses about which fish species are potential indicators of river quality? Discuss any differences in the three methods used. Do they agree or disagree? Explain.
...
The second (sites 15 - 19) and forth (sites 26 - 30) regions have been rated as being moderate quality.
...
Use a Mantel test, you could determine if $\beta$-diversity (i.e., Bray-Curtis distance) is correlated with environmental variables. 
...
Here we will compare the Bray-Curtic distance matrix we created earlier with a new distance matrix of environmental factors (doubs$env).
...
***Question 11***: What do the results from our Mantel tests suggest that about fish diversity and stream environmental conditions? How might this relate to your hypothesis about stream quality influencing fish communities?
