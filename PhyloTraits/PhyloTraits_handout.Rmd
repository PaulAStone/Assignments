...
table.phylo4d(x, treetype="clad", symbol="colors", show.node=TRUE, 
              ...
              col=mypalette(25), pch = 15, cex.symbol = 1, 
...
blom.K <- Kcalc(randtraits[randtree$tip.label],randtree, checkdata = TRUE)
randtree$tip.label <- nj.rooted$tip.label
randtree$edge.length <- nj.rooted$edge.length
Kcalc(nb, randtree)

# Calculated Phylogenetic Signal with `phylosignal` Function in the `picante` Package
blom.K <- Kcalc(nb, nj.rooted, checkdata = TRUE)

signal.nb <- phylosignal(nb[nj.rooted$tip.label], nj.rooted)

trait.list <- split(traits, rownames(traits))

signal.P <- sapply(traits, phylosignal, nj.rooted, reps = 999, checkdata = TRUE)
```
#apply through all of them. use b-h correction 
