...
To measure phylogenetic dispersion, we are going to use Blomberg's K statistic (Blomberg & Garland 2002). 
Blomberg's K calculates the amount of "phylogenetic signal" in the tip data relative to the expectation for a trait that evolved by Brownian motion along the specified topology and branch lengths. 
A value of 1.00 indicates exactly the amount expected by Brownian motion. 
Values < 1 indicate less resemblance of relatives than expected, and values > 1 indicate stronger-than-expected phylogenetic `clumping' of trait values. 
For trait 1, K=1.084, indicating that the tendency for related species to resemble each other is, averaged over the entire tree, just about what one would expect if the traits had actually evolved by a process similar to Brownian motion. 
To calculate K we will use the `phylosignal()` function in the picante pacakge.
