...
***Question 2***:  Make some observations about the `muscle` alingmnet of the 16S rRNA gene sequences for our bacterial isoaltes and the outgroup (*Methanosarcina*). 
...
Neighbor joining can serve as a starting point for exploring different evolutionary models using **maximum likelihood** methods, which we will explore in this handout. 
Last, phylogenetic trees can be constructed used **Bayesian** methods in the R package '`phangorn`.
...
In the R chunk below, the "model = raw"" argument means that `dist.dna` will estimate distances based on the proportion of sites that differ between pairs of sequences, while the "pairwise.deletion = FALSE" argument means that an entire site (i.e., column) is deleted if there is missing observation (i.e., a gap) for one or more of the sequences (i.e., rows). 
