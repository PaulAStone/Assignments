...
We enriched and isolated 39 strains of bacteria from two lakes. 
...
***Question 3***:  Using the saturation plot and cophylogenetic plots from above, describe the effect that that the F84 substitution model has on our phylogenetic reconstruction. 
...
# Add Row & Column Names to Nich Breadth Matrix
...
An emerging goal of biodiversity is to understand the effect that traits have on the peformance of species under different environmental conditions, but also how the traits of taxa can affect ecosytem functioning. 
Before attempting to establish relationships among traits, however, it is important to understand that the role that evolution plays in the distribution of traits. 
Because of hierarchical shared ancestry of species, we expect there to be non-independence of traits with respect to their evolutionary history. 
It is important that we test for the presence of this **phylogenetic signal** before drawing conclusions about patterns among traits. 
However, patterns of phylogenetic signal can be interesting and provide insight into ecolgoical and evoutionary processes that give rise to the **clustering** or **overdispersion** of traits, which are sometimes associated with **enviornmental filtering** and **competitive resuplsion**, respectivley.
In the following sections, we introduct some methods for testing hypotheses about the distribution of traits with respect to phylogeny. 
...
***Question 5***:  There are two important pieces of output from the `fitContinuous` function that can help you interpret the phylogenetic signal in trait data sets. 
...
# Use the BH correction on P-values: NEEDS TO BE FIXED -- CODE I MADE ISN'T RIGHT
# ALSO WE SHOULD DISCUSS DISCREPANCY/INTERPRETATION OF OUTPUT

p.phylosignal[6, ] <- round(p.adjust(p.phylosignal[4, ], method = "BH"), 3)
p.Kvalue <- round(p.phylosignal[1, ], 3)
p.phylosignal.out <- data.frame(p.Kvalue, p.phylosignal[6, ])
```
...
### C.  Calculate Dispersion on a Trait (Continuous)
Another way to calculate the phylogenetic signal of a trait (i.e., clustered vs. overdispersed) is to use a formal test of dispersion. 
In ecology, dispersion is a measure of how things (e.g., traits or species) are distrubuted over and area habitat (which we will consider next week). 
However, we can use similar approachs to determine how well traits are distributed within a phylogenetic tree.
One measure of trait dispersion is the *D* measure of phylogenetic signal strength (Fritz & Purvis, 2009).  
This measure of dispersion is calculated on catigorical data. 
For example, is an isolate able to growth using DNA is the sole phosphorus source?
Here, we will calcuate *D* using the `phylo.d` function in the `caper` package to test for phylogenetic dispersion of phosphorus usage in our bacterial strains. 
However, there are a few things we will need to do first: 
1) we need to turn our categorical data into continuos data,
2) we need to add a column with our isolate names, and
3) we need to combine our tree and trait data using the `comparative.data()` command in `caper`.
Final, we can calculate dispersion for each of our traits using `phylo.d`.
The output of this function will return and estimated value of D. 
Estimates of D can either be negative (clustered traits) or positive (overdispersed traits).
Furthermore, estimates of D being 0 indicate that the trait is clumped by randomly (Brownian motion) and estimtes of D being 1 indicate that a trait is randomly dispersed (random). 
In addition, the function will use a permutaion test to calculate the probability of the D being different from either 1 (no phylogenetic structure; random) or 0 (Browning phylogenetic structure).
Let's use this function to calculate dispersion on a few of our phosphorus traits.
...
***Question 7***: Using the estimates for D and the probabilities of each phylogenetic model, answer the following:

a.  Which phosphorus traits (try a few) are clustered, Brownian, random, and overdispersed?
b.  How do these results compare the results from Blomberg's K?
c.  Discuss why you might observe differences. What might effect these differences?
...
1.  Below is the output of a multiple regression model depicting the relationship between the maximum growth rate of each bacterial isolate and the niche breadth of that isoalte on the 18 different sources of phosphorus. 
One feature of the study which we did not reveal earlier in the handout is that the isolates came from two very different lakes. 
One of the lakes is an very oligotrophic (i.e., low phosphorus) ecosystem name Little Long (LL) Lake. 
The other lake is extremely eutrophic (i.e., high phosphorus) ecosystem named Wintergreen (WG) Lake.
Based on your knowledge of the the traits and their phylogenetic distributions, what conclusions would you draw about our data and evidence for a generalist-specialist tradeoff? 
