...
This is a very common file format that is used across platforms and disciplines. 
...
***Question 1***: Using nano or your favorite text editor, compare the `p.isolates.fasta` file and the `p.isolates.afa` file. Describe the differences that you observe between the two files. 
...
image.DNAbin(window, cex.lab = 0.6) # command to visusalize seq align {ape}
...
Not only is this effective for visualizing the evolutionary relationship among taxa, but as you will see later, the information that goes into a phylogenetic tree is needed for subsequent hypothesis testing. 
...
Instead, we wil empahasize distance-based approaches that are used for generating a tree, specifically **neighbor-joining**. 
Neighbor joining can serve as a starting point for assessing different evolutionary models using **maximum likelihood** methods, which we will explore in this handout. 
...
Neighbor joining trees are created from a distance matrix that is generated by comparing pairs of sequences.
...
This process is iteratively applied to the remaining sequences outside until the tree is resolved. 
...
The "pairwise.deletion = FALSE" argument means that an entire site (i.e., column) is deleted if there is missing observation (i.e., a gap) for one or more of the sequences (i.e., rows). 
...
In the process we will identify an outgroup sequence (*Methanosarcina*), which we will use to root the tree. 

```{r}
# Use Neighbor Joining Algorithim to Construct Tree (makes 'phylo' object {ape})
nj.tree <- nj(seq.dist.raw)

# Identify Outgroup Sequence and Reroot Tree
outgroup <- match("Methanosarcina", nj.tree$tip.label)
nj.rooted <- root(nj.tree, outgroup, resolve.root = TRUE)

# Use Rooted Tree to Plot Phylogeny
...
##MARIO: CAN YOU START ADDING TEXT TO FILL OUT THIS TABLE? CAN'T GET TO KNIT. ALSO, MAYBE WE DON'T NEED IF WE HAVE TEXT ABOVE? OR I GUESS WE PROBABLY DON'T NEED BOTH
...
```{r}
seq.dist.K80 <- dist.dna(p.DNAbin, model = "K80", pairwise.deletion = FALSE) # {ape}
```
...
###MARIO: CAN WE ANNOTATE SOME OF THIS R CHUNK?
```{r}
...
plot.phylo(K80.rooted, type = "phylogram", direction = "right", 
           use.edge.length = FALSE, adj = 0.5, cex = 0.7, label.offset = 1, main = "K80 tree")
...
***Question 3***:  Using the saturation plot and cophylogenetic plot, describe the effect that that the K80 substitution model has on our phylogenetic reconstruction. If the plots seem to be inconsistent with one another, explain what it giving rise to the differences.   
...
In this exercise, we will use maximum likelihood to determine which DNA substitution models best fits our sequence data. 
...
### B. Comparing a Few Models (just compare "raw" vs. "K80" as formal test of visualization above)
```{r}
...
outgroup <- match("Methanosarcina", tree$tip.label)
tree <- root(tree, outgroup, resolve.root=TRUE) # reroots the tree
fit.JC <- pml(tree, read.aln.phydat) # maximum likelihood function; 
          #returns data, tree, and output, etc. {phangorn}
fit.JC.opt <- optim.pml(fit.JC, TRUE) # optimizes model parms; default is Jukes-Cantor {phangorn}
...
#fitStart.1 <- eval(get("GTR+G+I", env), env)
#fitStart <- eval(get(best.model, env), env)
...
trait.nb <- trait.nb[!rownames(trait.nb) == "LL12", ]
