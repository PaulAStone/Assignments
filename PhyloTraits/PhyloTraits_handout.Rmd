...
### C.  Calculate Dispersion on a Trait
###SECTION NEEDS A LITTLE POLISHING AND PERHAPS ALSO A QUESTION. FOR EXAMPLE, ARE THE RESULTS CONSISTENT WITH BLOMBERG'S K?
Another way to calculate the dispersion of a trait (i.e., clustered vs. overdispersed) is to use a formal test of dispersion. 
Here, we will use the `phylo.d` function in the `caper` package to test for phylogenetic dispersion on the standardized growth rate of our bacterial strains. 
...
## 7) HOMEWORK
Below is the output of a multiple regression model depicting the relationship between the maximum growth rate of each bacterial isolate and the niche breadth of that isoalte on the 18 different sources of phosphorus. 
...
###WOULD LIKE TO HAVE INFO BELOW BLOCKED OUT. ONLY WANT THEM TO SEE THE FIGURE AND THE MULTILPE REGRESSION OUTPUT OF THE MODEL. 
