...
# Make Tree and Map Taits {adephylo}
...
table.phylo4d(x.nb, treetype="phylo", symbol="colors", show.node = TRUE, 
              ...
              edge.color = "black", edge.width = 2, box = FALSE, 
              col=mypalette(25), pch = 15, cex.symbol = 2, 
              ratio.tree = 0.90, cex.legend = 1, center = FALSE)
```

***Question 4***:  Based on the distribution of traits on the neighbor joining tree that you just created, what might you concldue about the degree of specialization of bacteria on diverse phosphorus resources?  

> ***Answer 4***:   

## 6) HYPOTHESIS TESTING
After one constructs and maps traits onto a tree, there are a number of questions and 
...
Let's look at what happens to our tree when we visualize our tree with a lamda of 1, 0.5, and 0. 
...
par(mfcol=c(1,3)) 
plot(nj.rooted, main = "lambda = 1")
plot(nj.lambda.5, main = "lamba = 0.5")
plot(nj.lambda.0, main = "lamba = 0")
```
