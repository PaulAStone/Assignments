...
2. Calculate Blomberg's K for each phosphorus resource using the function `phylosignal()`.
3. Use the Benjamin-Hochberg method to correct for false discovery rate.
4. Calculate Blomberg's K for nich breadt using the function `phylosignal()`.
...
3) Combine the tree and trait data using the `comparative.data()` command in `caper`.
