...
## Overview **<- We Need to use headers better, we are avoiding level 1 "#" because the font is large, but 2 "##" and 3 "###" are good. However, we need to use them in order. The document uses level 3 for main topics like evenness but puts all of this intro stuff at level 2. It may seem like little details, but this all caries over into the PDF and makes it look more professional. {MEM}**

In this exercise, we will use R and RStudio to explore aspects of local or site-specific diversity, also known as alpha ($\alpha$) diversity. **<-- Ken, I've changed the first few paragraphs, but I think it would be helpful to use the one-sentence-per-line format (I'll let you edit the rest) {MEM}**
We will use the tropical forest dataset from Barro-Colorado Island (BCI). 
BCI is a 1,560-hectare island in the center of the Panama Canal that is administered by the Smithsonian Tropical Research Institute. 
After setting up our R working environment, we will explore the BCI dataset and ways to quantify and examine the $\alpha$-diversity within individual plots of tropical trees.
...
Ecologists have developed many packages for conducting quantitative analyses in R. 
You can use the `help()` function to learn more about package installation and add-ons. 
In this class, we will often use the package called 'vegan', which contains tools commonly used in ecological research, including analyses of ($\alpha$)-diversity. 
...
In vegan, the BCI data are organized into a **site-by-species matrix**, that is, a data table where the abundances of all 225 tree species (columns) are given for each of the 50 sites (rows). **<- It would make more sense introduced in order (rows, then columns) {MEM}**
Consequently, this table should have 50 rows (sits) and 225 columns (species). 
...
```{r}
Site1 <- BCI[1,] # assign the first row (site) to the variable Site1 <-- the prefered notation for names is all lowercase. I know that I don't always do this, but a good habit to learn. Accepted identifiers like "S" would be an exception {MEM}
dim(Site1) # print the dimensions of Site1 to the screen 
```

You can see that Site1 has one row (it is only 1 site) and 225 columns (225 potentially present species). Remember that **<--However? {MEM}** the BCI site-by-species matrix also accounts for absences, i.e., with zeros. Let's find out how many species actually occupied Site1 when it was censused:
...
You could also print this in your final PDF document like this: `r S` **<-- not sure how to each this yet {MEM}**
