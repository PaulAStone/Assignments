...
Otherwise, we recommend you just use the `require` function to install dependencies.
...
There is also a function in the `vegan` package called `specnumber` also calculates observed richness. 
...
d.  What would we conclude from Good's Coverage if **$n_{1}$** equaled **$N$**?
...
soilbac <- as.matrix(t(soilbac), mode='numeric')
soilbac <- matrix(soilbac, ncol= ncol(soilbac), nrow=nrow(soilbac), dimnames = NULL)
soilbac1 <- soilbac[2,]
```
...
### MARIO: should we use which() and length() here?
```{r}
S.chao <- function(x){
...
### MARIO: I haven't spent much time on this. What goes into the fucntion for "community"?
```{r}
...
rarecurve(soilbac, step = 20, sample = min.N, col = "blue", cex = 0.6, las=1) #
abline(0, 1, col = 'red')             
...
One of the most common ways to visualize evenness is in a **rank-abundance curve (RAC)** (sometime referred to as a rank-abundance distribution or Whittaker plot).
...
plot(ranks, log(rac), type = 'p', las = 1, xlab = "Rank in abundance", ylab = "log(abundance)")
```

***Question 8***: What effect does log-transforming the abundance data have on how we interpret evennes in an RAC?
...
### Mario: it would be good stick with our `S.obs` funciton, but it doesn't seem to work below. Transpose vector?##
```{r}
SimpE <- function(x){
...
We can see that Simpson's evenness for `site1` is quite low (~0.01). 
However, Simpson's evenness has been criticized for being biased towards the most abundant taxa.
The value of the metric is sensitive to differences in the few most abundant species.
...
***Question 9***: Compare estimates of evenness for `site` of BCI using $E_{1/D}$ and $E_{var}$.
Do they agree?
If not, why?
...
Here, we will estimate popular indices of diversity, which explicitly inorporate richness and evenness. 
...
###MARIO: here we're introducing loops, perhaps for the first time. Should we use `apply` to follow on week1. Important
```{r}
H <- 0
for (sp in rac){
  p = sp / sum(rac)
...
### Is there a typo? "often expressed as 1/*D*... so that diversity naturally increases with 1/*D*."???
...
And now let's express Simpsons's diversity as 1/*D*
```{r}
invD <- 1/D 
```
...
Now we will use `vegan` to estimate Simpsons's index:
