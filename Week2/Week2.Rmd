...
###Adding Up Species In A Site Or Sample
...
Pick another site at BCI and address the questions above. {Homework}

###Richness Estimators

There are number of techniques that have been developed by biodiversity researchers to estimate species richness from species accumulation curves, which plot the number of species in a sample as a function of the number of individuals identified (e.g., [Hughes et al. 2001](http://aem.asm.org/content/67/10/4399). 

Here, we will hightlight to non-paremetric diversity estimators. 
The first is **Chao1**, which is calculated as the observed richness (S) plus the proportion of singletons to doubletons. 
This technique tends to be used in systems that are skewed towards low-abundance classes.
Let's right a function for Chao1:
...
want to 
  1) create vector of richness classes 1,2,3....25
  2) sum up number of observations at each richnnes class level

f<-1 #starting richness level
store<-NULL
for (f in 1:max(site1)) #counter that creates richness levels
  f<-f+1
  {  
  store<-sum((site1==f)*site1)  
   }  
...
for (n in 1:ncol(site1)){
  vec[n] = (site1==1)*site1 #singleton, doubleton, tripleton..
  
  f<-f+1  
  vec[k] = (site1==f)*site1 
  for (k in 1:ncol(site1))
}
```
...
Another commonly used non-parametric estimator of richness is **ACE**. This estimator requires the following:

thresh<-10
```{r}
S_abund <- rowSums(site1>thresh)*1 # Species richness when abundance greater than or equal to 10
S_rare <- rowSums((site1 <=thresh & site1>0)*1) # Species richness when abundances less than or equal to 10, but greater than zero; note S_abund + S_rare = S_total
N_rare <- sum((site1<=thresh)*site1) # Abundance (# of individuals) across rare species
C_ace <- 1 - sum(site1==1)/N_rare # ACE coverage; 1 minus singleton species divided by abundance of rare species; proportion of all individuals in rare species that are not singletons.
G_ace <- (S_rare/C_ace)
N_rare*(N_rare-1)*
...
Some additional thought is needed when estimating richness for multiple samples, especially if there is unequal sampling effort. 
