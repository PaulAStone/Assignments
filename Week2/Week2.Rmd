...
```
Writing and use of functions is central to programming.
...
There is also a function in the `vegan` package called `specnumber`. 

### JTL: new question for exercise:
### Question 2: Does `vegan` return the same value for observed richness of `site1` as the function that we just wrote (`S.obs `)?
...
, where **$n_{1}$** is the number of number of individuals belonging to singleton species, i.e. species encountered only once in a sample, and **$N$** is the total number of individuals in a sample. 
...
###Question 3:
a.  How many sequences did we recover from `soilbac1`?
...
Here, we will hightlight one of these, whichi is non-paremetric diversity estimators called **Chao1**
...
###Question 4:
a.  How does observed richness compare to estimated richness for `soilbac1` using `Chao1`?
...
```{r}
site1.ab.ranked <- site1.ab[order(site1, decreasing = TRUE)] # Use `order` function and "decreasing" argument to rank taxa by abundance
...
```{r}
D <- diversity(site1.ab, "simp") # find Simpson's diversity (D)
S <- specnumber(site1.ab)
...
### You're going from observed values (e.g. richness), to estimated values, and then back to observed values. -Ken
### Are you sure that you also want to go from BCI, to microbes, and back to BCI? -Ken
### Could be more linear by just going from observed to estimated and/or from BCI (or microbes) to microbes (or BCI). -Ken

k <- sample(nrow(BCI), 1) ## take a random site from BCI
### I would suggest not doing this because you might get a pretty crappy site. -Ken
### Best to have control over it, especially since students homework won't end up agreeing. -Ken
### Maybe just use the original site chosen from BCI (site1), which I had already checked for having a decent number of species. -Ken
fisher <- fisherfit(BCI[k,]) # using an unambiguous naming convention

```{r}
N <- sum(site1.ab) # find the number of individuals at site1 and assign it to a variable N
