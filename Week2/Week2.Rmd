...
###Question 1. Makes some additional observations about the occurrence and abundance of species in the samples you've indexed and print them on the lines provided. Do this by surrounding your answer in single or double quotes. 
...
However, estimating species richness for a community from an incomplete sample requires assumptions to be made about the nature of the sampling effort (e.g. biases, coverage).
Estimating richness is an area of intensive research and there are a few things to consider and pitfalls that we should avoid. 
...
One index that provides an estimate of how well a site is sampled is known as **Good's Coverage (C)**, which is defined as ... 
This is expressed as $1 - \frac{n_{1}}{N}$
, where **$n_{1}$** is the number of singleton species, i.e., for which only one individual was sampled, and **$N$** is the total number of individuals in a sample.
...
1.) Have the researchers at BCI done a good job of sampling `site1`? 2.) What is the range of values that can be generated by Good's Coverage? 3.) What portion of taxa in `site1` were represented as singletons?

```{r}
"1. Sure maybe? 2. from negative sideways 8 to positive sideways 8? 3. 100% of those with only one individual. "
```
...
###Questions:
3.  How many sequences did we recover from the sample `soilbac1`, i.e. the row total? 
4.  What is the observed richeness of `soilbac1`?
5.  How does coverage compare between the BCI sample (`site1`) and the KBS sample (`soilbac1`)?

There are number of techniques that have been developed by ecologists to estimate species richness based on the coverge of species in a sample (e.g., Hughes et al. 2001  -- http://aem.asm.org/content/67/10/4399).
Here, we will hightlight one of these, which is a non-parametric diversity estimator called **Chao1**.
