...
The site-by-species matrix is one of the most basic and common data structures used by ecologists. With the site-by-species matrix for BCI loaded into R, let's examine data in the first site (row).

```{r}
head(BCI)[, 14:16] # prints data for sites (rows) 1 through 2 and species (columns) 1 through 3
```

Using "head(BCI)" alone would print the entire dataset to the screen, so we've limited our output. By default, the to the first eight sites (rows) and the 14th to 16th species. But, as you can see, the "head" command defaults to printing "NA" after 6 rows. This is a good lesson in understanding what a function actually does and in checking what a function is actually doing. An alternative is to access the site-by-species matrix directly:  

```{r}
BCI[1:8, 14:16]	# prints data for sites (rows) 1 through 2 and species (columns) 1 through 3
```
