...
Even though we obtained a fair number of sequences from our samples, soil bacteria are thought to be some of the most divese communities on earth. 
...
soilbac <- setNames(data.frame(t(soilbac.raw[,-1])), soilbac[,1]) # transpose and turn column headers into row names so we have a site (rows)-by-"species" (columns) matrix
...
```{r}
S_chao <- function(x){
  rowSums(x>0)*1 + (sum(x==1)^2)/(2*sum(x==2)) ### should we use which() and length() here?
  }
```
...
```
### this is pretty hairy I guess. Opportunity to teach about loops and alternatives in R (sapply). Or we can just drop it and stick with Chao1
