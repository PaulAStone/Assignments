...
```{r}
site1 <- site1[site1>0] # Remove all zeros from Site1 and reassign the new vector
```

```{r}
site1.ranked <- site1z[order(site1, decreasing = TRUE)] # Use `order` function and "decreasing" argument to rank taxa by abundance
ranks <- as.vector(seq(1, length(site1)))             # Create a sequence of S ranks
plot(ranks, log(site1.ranked), type = 'p', las = 1,      # Plot the RAC
...
```{r}
P <- log(log(site1.ranked)) # log-tranform the abundances of the RAC and assign them to a vector P
