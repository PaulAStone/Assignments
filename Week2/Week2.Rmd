...
A next natural step in exploring aspects of local diversity is simply to ask how abundance varies among species. One of the most common ways to visually explore this relationship, commonly known as the **species abundance distribution** is simply to rank the species from greatest to least, that is, as a **rank-abundance curve (RAC)**. Let's plot the RAC for Site1.

Let's begin by first removing the zeros from Site1. This removes the species the species that, while in the site-by-species matrix, were not found at Site1.

```{r}
Site1 <- Site1[ !Site1 %in% c(0) ] # Remove all zeros
```

Now, we can plot the RAC, accounting only for the species that were found there.

```{r}
RAD <- Site1[order(Site1, decreasing=TRUE)] # Rank the taxa by abundance (y-values)
ranks <- seq(1, ncol(RAD)) # Create a sequence of S ranks (x-values)
plot(ranks, RAD, type='l', xlab="Rank", ylab="Abundance") # plot the RAC
```
