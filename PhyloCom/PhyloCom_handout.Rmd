...
com <- read.otu(shared = "data/INPonds.final.FW.shared", cutoff = "1")
...
# I don't want to add things manually outside of R. I'm thinking that we can just append the sequence to this file. I'll probably align an outgroup to silva first to make things easier though.
...
Then, the metric sums the branches lengths for each species in a sample from the root to the tip of the tree. 
...
There are two commonly used indices to test for phylogenetic clustering and overdispersion in a simple. 
The first is the Net Relatedness Index (NRI).
...
The resulting value is multiplied by -1 so that larger values correspond with overdispersion and smaller values correspond with clustering. 
Let's calucate NRI:
...
ses.mpd <- ses.mpd(comm, phydist, null.model = "taxa.labels", abundance.weighted = FALSE, runs = 99)
