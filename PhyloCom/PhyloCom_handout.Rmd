...
In this exercise, we will introduce some commonly used methods in phylogenetic community ecology.
=======
...
The source code file has user-defined functions that are required for certain analyses.
=======
In addition to relying on contributed packages, this week, we will also be using a source code file.
...
Conduct an exploratory analysis of the phylogenetic $\alpha$-diversity of the Indiana refuge ponds.
Using techniques from the *Week1_Handout.Rmd* and other statistical techniques at your disposal, identify enviornmental variables that may influence the taxonomic richness (SR), phylodiveristy (PD), and dispersion (NRI and NTI) of the bacterial communities in the sampled ponds. 
...
Additinally, we will calculate the amount of explained variation for each phylogenetically informed PCoA axis.
...
For many organisms, watershed boundaries represent major dispersal baries, which may influence the phylogenetic distribution of species. 
...
env.dis <- vegdist(scale(envs), method = "euclid")
```
...
You will recall that this constrained ordination technique allows one to test for the effects of an explanatory matrix (e.g., environmental data) on a response matrix (phylogenetic distance matrix). 
...
row.names(vectors) <- c("Elev", "Dia", "Dep", "Vol", "ORP", "Temp", "SpC", "DO", "TDS", "Sal", "pH", "Col", "Chla", "DOC", "DON")
...
First, we will calcualte distances for geographic data, taxonomic data, and phylognetic among all unique pair-wise combinations of ponds.

```{r}
# Geographic Distances Among Ponds
coord.dist <- dist(as.matrix(env$lat, env$long)) 

# Taxonomic Distances Among Ponds (Bray-Curits)
bray.curtis.dist <- 1 - vegdist(comm) 

# Phylogenetic Distances Among Ponds
unifrac.dist <- 1 - unifrac(comm, phy) 
...
par(mfrow=c(2, 1))
plot(geo.dist, bray.curtis, xlab="Geographic Distance (degree distance)", ylab="Bray-Curtis Similarity", 
   main = "Taxonomic Distance Decay", col='SteelBlue')
...
plot(geo.dist, unifrac, xlab="Geographic Distance", ylab="Unifrac Similarity", 
     main="Phylogenetic Distance Decay", col='darkorchid4')

DD.reg.uni <- lm(unifrac ~ geo.dist)
abline(DD.reg.uni, col="red4")
```

Finally, let's test whether or not the parameters (slopes and intercepts) for taxonomic and phylogenetic DD are significantly different from one another.

```{r}
diffslope(geo.dist, unifrac, geo.dist, bray.curtis)
diffic(geo.dist, unifrac, geo.dist, bray.curtis)
```

***Question 7***: Interpret the parameters from the taxonomic and phylogenetic DD relationships. If there are differences, hypothesize why this might be.   
...
```{r, echo=FALSE}
# I'm commenting this out as I'm not sure they need to rewrite this all. 
# Just want to jog their memories. Not a direct comaparison like other chunks in other sections. 
SAR <- function(OTUs){
  Alist <- c()
  Slist <- c()
...
PDAR <- function(comm, tree){
  ...
  for (i in num.plots){    
    ...
    for (j in 1:10){ # Iterate 10 times for each sample size
      ...
      for (k in pond.sample) { # Loop through each randomly drawn pond  
        area <- area + length(pond.sample) # aggregating area
        sites <- rbind(sites, comm[k, ]) # and sites
        }
      
      areas.iter <- c(areas.iter, area) # concatenate the area to areas.iter      
      psv.vals <- psv(sites, tree, compute.var=FALSE) # calculate PSV or other metric (e.g., UniFrac)
      psv <- psv.vals$PSVs[1] 
...
plot(pdar[, 1], pdar[, 2], xlab='Area', ylab='PSV', xlim = c(0 , 52), ylim = c(0, 1), 
     main='Phylogenetic Diversity-Area Relationship', col='red', pch = 16, las = 1)
...
***Question 8***: For the bacteria in the Indiana ponds, the slope (*z*) of the SAR was 0.15.
