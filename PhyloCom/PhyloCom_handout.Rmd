...
pdac.psv <- function(comm, tree, PD.index='PSV'){
  ...
  for (i in num.plots){    
    ...
    for (j in 1:100){ # Iterate 100 times for each sample size
      ...
      sites <- NULL # declare a null object
...
P.vals <- cor(pdac, method="pearson")
P <- round(P.vals[2],3)
S.vals <- cor(pdac, method="spearman")
S <- round(S.vals[2],3)

plot(pdac, xlab='Area', ylab='PSV', main='Phylogenetic diversity-area relationship', col='red')
legend("topleft", legend=paste("Spearman correlation =", S,
                               "\nPearson correlation =", P), bty="n", lw=0, col = "red") 
