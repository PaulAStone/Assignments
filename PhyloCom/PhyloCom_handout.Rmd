...
### JTL: perhpas we can work with a reduced commmunity data set (a few sites and a subset of the taxa) to address the slow performance of ses.pd
### JTL: the other thing to do is to carefully compared the types of data that are going into the function (comm and phy)
### JTL: last, Mario suggested that maybe it has something to do with the null.model
...
# The off-diagonal terms are covariances; sum((Xi - Xm)*(Yi - Ym))  --Ken
# Since the variance is the covariance of a term with itself, the diagonal terms are variances: sum((Xi - Xm)*(Xi - Xm)) --Ken
# The covariance is the numerator in Pearson's correlation coefficient: cov(X,Y)/(std(X)*std(Y))
# So, multiplying Pearson's correlation coefficient by the standard deviations of X and Y will yield the covariance. --Ken
# Does the code use the (co)variance or sample (co)variance?  --Ken
...
Helmus and Ives use the Phylogenetic species variability (PSV) metric.
PSV quantifies how phylogenetic relatedness decreases the variance of a hypothetical unselected (i.e. neutral) trait shared by all species in a community. 
...
```{r}
pdac.psv <- function(comm, phy.rooted, PD.index, spatial=FALSE){
  
  # This function will accept the following arguments:
    # comm     :  the site-by-species matrix 
    # phy.rooted  : the rooted phylogenetic tree
    # spatial  :  aggregate randomly or with respect to distance
  
  reps=100
  areas <- c()
  diversity <- c()
  
  for (i in 1:51) { # Loop through all values of sample size
    ...
    for (j in pond.sample) { # Loop through each randomly drawn pond
      area <- area + pond.areas[j] # aggregating area
      sites <- rbind(sites, as.numeric(OTUs[j, ]) # accumulating abundances of OTUs, and new OTUs
      }

    
    if (PD.index == 'PSV')
      ratio <- rbind(ratio, psv(, phy, compute.var=FALSE))
