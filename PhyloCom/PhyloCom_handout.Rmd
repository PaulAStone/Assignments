...
pdac.psv <- function(comm, tree, PD.index='PSV'){
  ...
  num.plots <- c(2,4,8,16,24,48) # increase numbers of plots sampled by 2
  # This will roughly double area
  ...
  for (i in num.plots){ # Loop through all values of sample size
    ...
    for (j in 1:100){ # Iterate 100 times for each sample size
      ...
      pond.sample <- sample(51, replace = FALSE, size = i) # random sampling w/out replacement
...
areas <- pdac$areas
diversity <- pdac$diversity

cor.vals <- cor(pdac, method="pearson") 
cor.vals[2]
cor(pdac, method="spearman")


legend("topleft", legend=paste("Spearman correlation =", exponent), bty="n", lw=2, col = "red") 
