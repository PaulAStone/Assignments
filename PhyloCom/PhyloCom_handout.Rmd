...
Higher PD values indicate that an assemblage contains more evolutionary divergent taxa, while lower PD values indicate that an assemblage contains taxa with a more restricted evolutionary history. 
...
\begin{tabular}{ m{5cm}  m{10cm} }
  ...
  The average phylogenetic distance between two taxa calculated as the sum of all intervening branch lengths. "Mean phylogenetic distance between pairs of species drawn from two distinct communities.\\
  \\ [-1.5ex]
  \textbf{UniFrac} & 
  The distance between the two samples calculated as (the sum of *unshared* branch lengths)/(the sum of all [shared and unshared] tree branch lengths) in a rooted tree. \\
  \\ [-1.5ex]
  \hline
\end{tabular}
\end{center}
...
Here we are calcuating unweighted phylogenetic distances (similar to incidence based measures).
That means that we are not taking into account the abundance of each taxa in each site.

### B. Visualizing Phylogenetic Beta Diversity

Now that we have our phylogenetically based community resemblance matrix, we can visualize $\beta$ diversity using the same techniques we used in the $\beta$-diversity module.
...
Additinally, we we calculate the amount of explained variation for each phylogenetically informed PCoA axis.
...
# Mantel Test: Is UniFrac Distance Correlated with Environmental Variation?
mantel(dist.uf, env.dist)

# Conduct Canonical Correspondence Analysis
...
### B. PHYLOGENETIC DISTANCE-DECAY RELATIONSHIP (PDD)
...
```{r}
unifrac.dist <- 1 - unifrac(comm, phy) # unifrac distance
bray.curtis.dist <- 1 - vegdist(comm) # Bray-Curtis similarity between the plots
