...
coord.dist <- earth.dist(long.lat, dist=TRUE) # in kilometers
...
par(mfrow=c(2, 1))
plot(geo.dist, bray.curtis, xlab="Geographic Distance, km", ylab="Bray-Curtis Similarity", 
...
plot(geo.dist, unifrac, xlab="Geographic distance, km", ylab="Unifrac Similarity", 
...
PDAR <- function(comm, tree){
  ...
  num.plots <- c(2, 4, 8, 16, 32, 51) # increase numbers of plots by 2x
  ...
  for (i in num.plots){    
    ...
    for (j in 1:10){ # Iterate 10 times for each sample size
      ...
      psv.vals <- psv(sites, tree, compute.var=FALSE) # calculate PSV or other phylogenetic alpha-diversity metric
      psv <- psv.vals$PSVs[1] 
