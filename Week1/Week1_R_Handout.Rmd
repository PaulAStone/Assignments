...
During the course, we will useRStudio (http://www.rstudio.com/), which is a user-friendly integrated developement environment (IDE) that allows R to interface with other tools. 
...
In this case, the number inside of the square brackets tells R that we want call the ith element of vector `z`:
```{r}
z[2]
```
...
What does the `rnorm` function do? What are the arguments specifying? 
