...
***Here is my example (take it or leave it) - MEM***
...
In the R code chunk below, please calculate the following: the volumn of a cube with length, l, = 5, the area of a circle with radius,r, = 2 ($area = \pi * r^2$), the length of the opposite side of a right-triangle given that the angle = \pi/4 (radians, AKA 45°) and with hypotenuse lenght sqrt(2) (remember: $sin(\theta) = opposite/hypotenuse$), and the log (base e) of your favorite number:
...
In the R code chunk below, assign any value to the object "a".

***I can't really think of how to make this clever - MEM***

When you are, clear your R working environment using the `rm()` function.
...
1) Assign 5 user defined values to the vector x.
2) Create a vector w that is 5 times x.
3) Add x and w and devide by 6
...
2) Multiply k by x
3) Combine w and k into a third vector that consists of three elements from w and four elements of k
...
**Question 1:*** What did we do in the second `sem` function provided in the handout? Use `help()` if necessary.
> **Answer 1:*** 


###################### I'm going to stop here for now to see what you think. 
######################   Yes, I imagine this changing a bit below as we get into some actual data (part 6)
