...
## Overview
In this exercise, you will explore among-site biodiversity, also known as beta ($\beta$) diversity. 
...
Finally, you will be introduced to some multivariate techniques to test hypotheses about beta ($\beta$) diversity. 
...
setwd("~/GitHub/QuantitativeBiodiversity/Assignments/Week3") #sets the working directory
```

### Install Packages 

From last week's exercises, you should now be familiar with some of the features in the R package `vegan`.
This week, in addition to using `vegan`, we will also install `ade4` and `BiodiversityR`.
You can use the 'help' funciton to learn more about each package. 
##Are we using any functions from ade4 and BiodiveristyR? If not, should we just have them import the data from their directory? {JTL}

```{r, results = 'hide', message = FALSE, warning = FALSE} 
# should we be using above hide messages for all grey coding regions? want to be consistent across weeks {JTL}
...
The `BiodiversityR` package contains some BCI environmental data (BCI.env). 
Your Weeek3 folder has soil data for each of the BCI plots (bci.soil.txt).
...
## 3) Exploring Beta ($\Beta$) Diversity
Last week, we learned about alpha ($\alpha$) diversity, where we calculated things like species richness and evenness. 
...
```{r}
BCI.S.color <- rev(terrain.colors(151)) ## what about max(BCI.S)
```
...
This is why we used 151 instead of 150.
...
+ How does this compare to the patterns of species richness?
...
## 4) Quantifying Beta ($\Beta$) Diversity
The previous section was intended to visually demonstrate that aggregate measures of biodiversity -- like richness -- do not always capture meaningful differences in species composition among sites. In this section, we will introduce some of the ways in which beta ($\Beta$) diversity is directly quantified. 

### Turnover
One way to think about  beta ($\beta$) diversity is the change in communities over time or space. 
...
## Is the "census" verbiage necessary? Can we just say "sample"?
...
```{r}
turnover(doubs$fish[1,], doubs$fish[2,]) # change to BCI?
```
...
```{r}
beta.w(doubs$fish[1, ], doubs$fish[2, ]) # change to BCI?
```
...
##I'm a little confused about some of the precautions. We introduce potential pitfalls, but don't explain why they are pitfalls or how to avoid them. 
...
## should we include Euclidean since it's sort of an outlier? perhaps save for PCoA where it's easy to implement and visualize
In `vegan`, distances are calculated as `D = 1 - S`; however, this conversion may not preserve Euclidean distance.
