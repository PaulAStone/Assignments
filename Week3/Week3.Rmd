...
setwd("~/GitHub/QuantitativeBiodiversity/Assignments/Week3") #sets working directory
```

### Load Packages 

From last week's exercises, you should now be familiar with some of the features in the R package `vegan`.
We will again be using this pacakge and can load it now.
...
require(ade4)
data(doubs)
str(doubs, max.level = 1)                 # max.level limits the str output
```
...
In this section, we will introduce some of the ways in which beta ($\Beta$) diversity is directly quantified. 
...
Though given enought time we could code functions for all of these measures, many of the similarity metrics are included in the vegan `vegdist()` function.
...
### Cluster Analysis
Another common way to visualize $\beta$-diversity is to use a clusering diagram
